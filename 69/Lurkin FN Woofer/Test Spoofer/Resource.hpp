...
void notitlechange()
{
    SetConsoleTitleA("  LurkedModz  |  Free HWID Spoofer");
}
void title_changed()
{
    while (TitleChanging == true)
    {
        std::string consoleTitle = (std::string)E("  LurkedModz  |  ") + random_string();
        LI_FN(SetConsoleTitleA)(consoleTitle.c_str());
    }

    if (TitleChanging == false)
    {
        notitlechange();
    }
}
